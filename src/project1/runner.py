...
def add_observation_noise(asteroid_locations, noise_sigma_x=0.0,
    ...
    :param asteroid_locations dict(int: tuple(int, int)): A dictionary where the key represents the asteroid id, and the values
    are a tuple representing the (x, y) coordinates of the asteroid.
    :param noise_sigma_x (float): Noise for parameters in the x-axis
    :param noise_sigma_y (float): Noise for parameters in the y-axis
    :return:
    """
...
def run_estimation(asteroidshower,
    ...
    :param asteroidshower (object): A collection of Asteroids
    :param arena (object): An object representing the arena within which the asteroids operate. arena.bounds should return a dictionary with the x bounds and y bounds.
    :param noise_sigma_x (float): The noise for parameters in the x-axis
    :param noise_sigma_y (float): The noise for parameters in the y-axis
    :param asteroid_match_range (float): The distance within which true values and estimated values are considered to be matching (i.e. the circle around an asteroid).
    :param spaceship (object): An object with methods completed and returned by the student
    :param time_limit (int): Time period for the simulation to run.
    :param agent: N/A
    :param weights: N/A
    :param display:
    :param case_id:
...
def run_jumps(asteroidshower,
    ...
    :param spaceship (object): An object with methods completed and returned by the student
    :param time_limit (int): Time period for the simulation to run.
    :param agent (object): The spaceship attempting to cross the field
    :param weights:
    :param display:
    :param case_id:
    :return:
    ...
    for t in range(time_limit):
        ...
        if selected_idx is not None:
            if selected_idx in asteroids_true_ts_in_future:
                ...
                if ridden_asteroid:
                    if selected_idx == ridden_asteroid:
                        print("You are jumping to the same asteroid that you are already on, please return None instead.")
                    ...
                    double_jump = True
                    # if not riding asteroid, must be at start location
                    spaceship_xypos_fut = xypos_start
...
def spaceship_reached_goal(agent_y, goal, t, ret=None):
    ...
    :param t (int): The time (representing the iteration point within the time_limit loop)
    :param ret (tuple): A tuple of the message and time (for display purposes)
    :return:
    """
    message = FAILURE_TIME_STEP_LIMIT_EXCEEDED
    ret = (message, t)
...
def asteroid_in_jump_range(agent_fut_pos, asteroid_fut_pos, spaceship_jump_distance, double_jump):
    ...
    :param agent_fut_pos:
    :param asteroid_fut_pos:
    :param spaceship_jump_distance:
    :param double_jump:
    :return:
    """
    jump = False
