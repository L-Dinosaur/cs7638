...
def q4_move(self, motion):
    ...
    # This function should be implemented as a method within the robot class.
    res = robot(self.length)
    res.move_func = self.move_func
    d = motion[1]
    alpha = motion[0]
    ...
    if abs(beta) < 0.001:
        ...
        x_new = cx + sin(theta + beta) * R
        y_new = cy - cos(theta + beta) * R
        theta_new = (theta + beta) % (2 * pi)

    res.set(x_new, y_new, theta_new)
...
def q5_sense(self, add_noise=1):
    ...
    for i, landmark in enumerate(landmarks):
        ...
        Z.append((atan2(dy, dx) - self.orientation) % (2. * pi))
